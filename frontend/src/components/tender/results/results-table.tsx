...
import { evaluationRows as staticEvaluationRows, type EvidenceItem } from "@/data/results";
...
export const ResultsTable = ({ evidenceItems }: ResultsTableProps) => {
  ...
  const filteredRows = useMemo(() => {
    if (evidenceItems && evidenceItems.length > 0) {
      ...
      const groupedByCriteria = filtered.reduce(
        (acc, item) => {
      ...
      return Object.entries(groupedByCriteria).map(([criteria, bidders]) => ({
        criteria,
        bidders,
      }));
    }

    return staticEvaluationRows
      .map((row) => {
